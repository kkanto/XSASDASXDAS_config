...
def job_config = [
        job: [
                name: "XSASDASXDAS_Pipeline_develop"
        ],
        git: [
                branch: "develop"
        ]
...
def customLib = library identifier: 'XSASDASXDAS_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/XSASDASXDAS_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = lib.de.allianz.XSASDASXDAS.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                    jslGit.checkout( config, "JEQP", "XSASDASXDAS_Pipeline_develop", "develop")
                }
            }    
        }
        stage('Build') {
